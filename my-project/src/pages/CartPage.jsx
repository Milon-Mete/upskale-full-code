...
import { 
  Trash2, ArrowRight, Smartphone, Lock, Loader2, X, Tag, Video, Users, CheckCircle2, AlertCircle, ShieldCheck, Gift 
} from 'lucide-react';
...
const [appliedCoupon, setAppliedCoupon] = useState(null); 
  const [appliedPromotions, setAppliedPromotions] = useState([]); // 🔴 ADDED: Tracks system rules
  const [showSurpriseModal, setShowSurpriseModal] = useState(false); // 🔴 ADDED: Controls the popup
...
// 🔴 UPDATED: Calculate both base discount and surprise rules
...
if (res.ok && data.success) {
            ...
            setCouponMsg({ type: 'success', text: `Saved ₹${data.totalDiscount} Total!` });
            
            // 🔴 NEW: If the array has items, the system triggered a rule. Show the popup!
  ...
  const removeCoupon = () => { 
      setAppliedCoupon(null); 
      setAppliedPromotions([]); // 🔴 ADDED: Clear surprise discounts
      setCouponCode(''); 
 ...
 const handlePayment = async () => {
    ...
    try {
      // 🔴 REPLACED FETCH WITH AXIOS TO TRIGGER GLOBAL INTERCEPTOR ON 401
        ...
        handler: async function (response) {
          try {
            ...
            if (verifyData.success) {
             ...
             // Handle verification specific failures
      ...
      rzp.open();
        ...
        // If it's a 401/403, the App.jsx interceptor will handle the redirect. 
        // For other errors (like 400 Bad Request or 500 Server Error), show this alert.
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-100 font-sans relative pb-24">
              ...
              <div key={item.id} className="bg-[#0a0a0a] border border-white/10 hover:border-white/20 transition-colors rounded-3xl p-5 sm:p-8 shadow-2xl relative overflow-hidden group">
                ...
                {(item.itemModel === 'Course' || item.itemModel === 'Cohort' || item.itemModel === 'BiteSizeCourse') && (
                    <div className="space-y-6 border-t border-white/10 pt-6">
                        ...
                        {/* Payment Options (Full vs Installment) */}
                        {pref.activeTab === 'live' && item.itemModel !== 'BiteSizeCourse' && (
                            <div className="grid sm:grid-cols-2 gap-4">
                                    {pref.isPart2Payment ? (
                                        <div className="col-span-full bg-gradient-to-r from-yellow-500/10 to-amber-500/5 border border-yellow-500/30 rounded-2xl p-5 flex justify-between items-center relative overflow-hidden">
                                            <div className="absolute left-0 top-0 w-1 h-full bg-yellow-500"></div>
                                            <div>
                                                <h3 className="font-bold text-yellow-500 text-lg mb-1">Complete Your Payment</h3>
                                                <p className="text-gray-400 text-sm">Pay the remaining balance to unlock full access.</p>
                                            </div>
                                            <span className="font-black text-3xl text-white">₹{part2Price?.toLocaleString() || "0"}</span>
                                        </div>
                                    ) : (
                                        <>
                                            <label 
                                                onClick={() => updateItemPref(item.id, 'paymentMethod', 'full')}
                                                className={`relative flex flex-col p-5 rounded-2xl border-2 transition-all cursor-pointer group ${pref.paymentMethod === 'full' ? 'border-[#008a45] bg-[#008a45]/5' : 'border-white/5 bg-black/40 hover:border-white/20'}`}
                                            >
                                                <div className="flex items-center justify-between mb-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${pref.paymentMethod === 'full' ? 'border-[#008a45]' : 'border-gray-500 group-hover:border-gray-400'}`}>
                                                            {pref.paymentMethod === 'full' && <div className="w-2.5 h-2.5 bg-[#008a45] rounded-full"></div>}
                                                        </div>
                                                        <span className="font-bold text-white text-lg">Pay in Full</span>
                                                    </div>
                                                </div>
                                                <span className="font-black text-2xl text-white mb-2">₹{(pricing.live?.discount || item.price || 0).toLocaleString()}</span>
                                                <span className="text-xs text-gray-500 font-medium">One-time payment, lifetime access.</span>
                                            </label>

                                            {pricing.installment?.enabled && (
                                                <label 
                                                    onClick={() => updateItemPref(item.id, 'paymentMethod', 'installment')}
                                                    className={`relative flex flex-col p-5 rounded-2xl border-2 transition-all cursor-pointer group ${pref.paymentMethod === 'installment' ? 'border-[#008a45] bg-[#008a45]/5' : 'border-white/5 bg-black/40 hover:border-white/20'}`}
                                                >
                                                    <div className="absolute top-0 right-0 bg-[#008a45] text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl rounded-tr-xl uppercase tracking-wider">EMI Available</div>
                                                    <div className="flex items-center justify-between mb-4">
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${pref.paymentMethod === 'installment' ? 'border-[#008a45]' : 'border-gray-500 group-hover:border-gray-400'}`}>
                                                                {pref.paymentMethod === 'installment' && <div className="w-2.5 h-2.5 bg-[#008a45] rounded-full"></div>}
                                                            </div>
                                                            <span className="font-bold text-white text-lg">Pay in Parts</span>
                                                        </div>
                                                    </div>
                                                    <span className="font-black text-2xl text-white mb-2">₹{part1Price?.toLocaleString() || "0"} <span className="text-sm font-medium text-gray-500">/today</span></span>
                                                    <span className="text-xs text-gray-500 font-medium">Balance ₹{part2Price?.toLocaleString() || "0"} due later.</span>
                                                </label>
                                            )}
                                        </>
                                    )}
                        ...
                        {(pref.activeTab === 'recorded' || item.itemModel === 'BiteSizeCourse') && !pref.isPart2Payment && (
                  ...
                  <div className="space-y-4 mb-6">
                      ...
                      {appliedCoupon && (
                          <div className="flex justify-between text-[#00d26a] font-medium">
                              ...
                              <span>- ₹{appliedCoupon.baseDiscount.toLocaleString()}</span>
                          </div>
                      )}
                      
                      {/* 2. Automated Surprise Discounts UI */}
                      {appliedPromotions.length > 0 && appliedPromotions.map((promo, idx) => (
                          <div key={idx} className="flex justify-between text-yellow-500 font-bold items-center">
                              <span className="flex items-center gap-1">
                                  <Tag size={14} /> Extra: {promo.message || promo.promotionName}
                              </span>
      ...
      {/* --- SURPRISE PROMOTION POPUP MODAL --- */}
      {showSurpriseModal && appliedPromotions.length > 0 && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShowSurpriseModal(false)}></div>
          
          <div className="w-full max-w-sm bg-[#0a0a0a] border border-[#00d26a]/30 rounded-[2rem] p-8 shadow-[0_0_50px_rgba(0,210,106,0.15)] relative z-10 text-center transform transition-all animate-fade-in-up">
              {/* Glow Effect */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-[#00d26a]/20 blur-3xl rounded-full pointer-events-none"></div>
              
              <button onClick={() => setShowSurpriseModal(false)} className="absolute top-5 right-5 text-gray-500 hover:text-white transition-colors bg-white/5 p-2 rounded-full">
                  <X size={16}/>
              </button>

              <div className="w-20 h-20 bg-gradient-to-br from-[#008a45] to-[#00d26a] rounded-3xl mx-auto flex items-center justify-center mb-6 shadow-lg shadow-[#008a45]/40 animate-bounce">
                  <Gift className="text-white" size={40}/>
              </div>

              <h2 className="text-3xl font-black text-white mb-2 uppercase tracking-tight">Congratulations!</h2>
              <p className="text-gray-400 mb-6 font-medium text-sm">You just unlocked a hidden surprise discount.</p>

              <div className="bg-black/50 border border-white/5 rounded-2xl p-5 mb-8">
                  {appliedPromotions.map((promo, idx) => (
                      <div key={idx} className="flex justify-between items-center text-lg font-black text-[#00d26a] border-b border-white/5 last:border-0 pb-3 last:pb-0 mb-3 last:mb-0">
                          <span className="flex flex-col items-start">
                              <span className="text-xs text-gray-500 font-bold tracking-widest uppercase mb-1">Unlocked</span>
                              <span>{promo.message || promo.promotionName}</span>
                          </span>
                          <span className="text-2xl">- ₹{promo.discountValue}</span>
                      </div>
                  ))}
              </div>

              <button onClick={() => setShowSurpriseModal(false)} className="w-full bg-[#008a45] hover:bg-[#00d26a] text-white py-4 rounded-xl font-bold text-lg transition-all active:scale-95 shadow-lg">
                  Awesome, thanks!
              </button>
          </div>
        </div>
